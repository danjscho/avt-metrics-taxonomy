...
class TestCheckTierTotals:
    def test_pass(self):
        ...
        # shape it expects (52 / 90 / 79 as of v5.3.0 Phase 5 promotions).
        ms = (
            [_make_metric(ref_id=f"TP.AC-{i}", tier=1) for i in range(1, 53)]
            + [_make_metric(ref_id=f"TP.AC-{i}", tier=2) for i in range(53, 143)]
