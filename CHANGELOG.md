...
- **`taxonomy/tools/snapshot.py`** — Wayback Save Page Now driver. Reads `_references.md`, snapshots every entry whose `Archive:` field is empty, throttled and retried per Wayback rate limits. Idempotent. **Note: as of v3.9 release, anonymous Save-Page-Now submissions are rejected with HTTP 401; the pass requires Internet Archive S3-style credentials (`IA_ACCESS_KEY` / `IA_SECRET_KEY` env vars). The 100 catalogue Archive: fields remain placeholder until those credentials are supplied and snapshot.py is re-run; `audit.py check_archive_present` flags this at INFO.** Documented as v3.9.x snapshot pass; not blocking the v3.9 release because the catalogue, handles, citations, audit, and builds are all in place.
